...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // =========
  // QUBIT DEMO (RABI OSCILLATION + NOISE)
  // =========
  
  const qubitCanvas = document.getElementById("qubit-canvas");
  const qubitNoiseSlider = document.getElementById("qubit-noise");
  const qubitNoiseValue = document.getElementById("qubit-noise-value");
  const qubitToggleBtn = document.getElementById("qubit-toggle");
  
  let qCtx;
  let qRunning = false;
  let theta = 0; // The Rabi angle (0 to PI)
  let phi = 0;   // Azimuthal angle (rotation around Z)
  
  const initQubitCanvas = () => {
    if (!qubitCanvas) return;
    const parentWidth = qubitCanvas.parentElement
      ? qubitCanvas.parentElement.clientWidth
      : 480;
    const width = Math.min(parentWidth, 520);
    const height = width * 0.75; // 4:3 aspect
    qubitCanvas.width = width;
    qubitCanvas.height = height;
    qCtx = qubitCanvas.getContext("2d");
    drawQubitFrame(0); // Initial draw
  };
  
  const drawQubitFrame = (noiseLevel) => {
    if (!qCtx || !qubitCanvas) return;
    const { width, height } = qubitCanvas;
    const cx = width / 2;
    const cy = height / 2;
    const r = height * 0.35; // Radius of sphere
  
    qCtx.clearRect(0, 0, width, height);
  
    // 1. Draw Bloch Sphere Outline (Circle)
    qCtx.beginPath();
    qCtx.strokeStyle = "rgba(148, 163, 184, 0.3)";
    qCtx.lineWidth = 1;
    qCtx.arc(cx, cy, r, 0, Math.PI * 2);
    qCtx.stroke();
  
    // 2. Draw Equator (Elliptical)
    qCtx.beginPath();
    qCtx.ellipse(cx, cy, r, r * 0.3, 0, 0, Math.PI * 2);
    qCtx.stroke();
  
    // 3. Draw Axis (Z-axis)
    qCtx.beginPath();
    qCtx.moveTo(cx, cy - r - 10);
    qCtx.lineTo(cx, cy + r + 10);
    qCtx.strokeStyle = "rgba(148, 163, 184, 0.1)";
    qCtx.stroke();
  
    // Labels |0> and |1>
    qCtx.fillStyle = "rgba(255,255,255,0.8)";
    qCtx.font = "14px monospace";
    qCtx.textAlign = "center";
    qCtx.fillText("|0⟩", cx, cy - r - 15);
    qCtx.fillText("|1⟩", cx, cy + r + 25);
  
    // 4. Calculate Vector Position with Noise
    // Ideal Rabi: theta increases linearly
    // Noise: Jitters theta and phi
    let noisyTheta = theta;
    let noisyPhi = phi;
  
    if (noiseLevel > 0) {
      // Noise adds random jitter to the angle
      noisyTheta += (Math.random() - 0.5) * noiseLevel * 5; 
      noisyPhi += (Math.random() - 0.5) * noiseLevel * 10;
    }
  
    // Convert spherical to 2D projection
    const vx = cx + r * Math.sin(noisyTheta) * Math.cos(noisyPhi);
    const vy = cy - r * Math.cos(noisyTheta); // Minus because Y grows down
  
    // Draw Vector Arrow
    qCtx.beginPath();
    qCtx.strokeStyle = "#f97316"; // Orange accent
    qCtx.lineWidth = 3;
    qCtx.moveTo(cx, cy);
    qCtx.lineTo(vx, vy);
    qCtx.stroke();
    ...
    // Draw Vector Tip (Dot)
    qCtx.beginPath();
    qCtx.fillStyle = "#f97316";
    qCtx.arc(vx, vy, 5, 0, Math.PI * 2);
    qCtx.fill();
    
    // Draw "Shadow" if noisy to show where it SHOULD be (ideal path)
    if (noiseLevel > 0.1) {
       const idealVy = cy - r * Math.cos(theta);
       qCtx.beginPath();
       qCtx.fillStyle = "rgba(56, 189, 248, 0.3)"; // Blue ghost
       qCtx.arc(cx, idealVy, 3, 0, Math.PI * 2); 
       qCtx.fill();
    }
  };
  ...
  const animateQubit = () => {
    ...
    // Increment theta (Rabi frequency)
    theta += 0.03; 
    
    // Wrap around 2PI
    if (theta > Math.PI * 2) theta -= Math.PI * 2;
    
    const noise = qubitNoiseSlider ? parseFloat(qubitNoiseSlider.value) : 0;
    
    drawQubitFrame(noise);
    requestAnimationFrame(animateQubit);
  };
  
  if (qubitCanvas) {
    initQubitCanvas();
    window.addEventListener("resize", initQubitCanvas);
  }
  
  if (qubitNoiseSlider && qubitNoiseValue) {
    qubitNoiseSlider.addEventListener("input", () => {
      qubitNoiseValue.textContent = Number(qubitNoiseSlider.value).toFixed(2);
      if (!qRunning) drawQubitFrame(parseFloat(qubitNoiseSlider.value));
    });
  }
  
  if (qubitToggleBtn) {
    qubitToggleBtn.addEventListener("click", () => {
      qRunning = !qRunning;
      qubitToggleBtn.textContent = qRunning ? "❚❚ Pause" : "▶ Start";
      if (qRunning) {
        animateQubit();
      }
    });
